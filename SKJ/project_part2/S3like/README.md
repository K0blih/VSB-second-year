...
FastAPI object storage gateway with bucket-based metadata, Alembic migrations, advanced transfer billing, soft delete, Message Broker integration, Haystack-backed binary storage, and a browser UI.
...
BROKER_WS_URL=ws://localhost:8000/broker HAYSTACK_BASE_URL=http://localhost:8002 uvicorn app.main:app --reload --port 8001
```
...
- File contents are sent to the Message Broker on `storage.write` and physically stored by the Haystack node.
- File metadata is stored in `data/files.db`, including upload `status`, `volume_id`, `offset`, and `size`.
- `POST /files/upload` returns `202 Accepted` while the object is still `uploading`.
- A startup background task listens on `storage.ack` and marks objects `ready`.
- `GET /files/{id}` proxies ready object bytes from `HAYSTACK_BASE_URL`.
...
Start the Haystack node in another terminal:
