...
TEST_SUITE("Iterators") {
    ...
    TEST_CASE("Filter") {
        ...
        auto filtered = arr
            .iter()
            .filter([](const int item) { return item % 2 == 0; });
    ...
    TEST_CASE("Map") {
        ...
        auto mapped = arr
            .iter()
            .map([](const int item) { return item * 2; });
    ...
    TEST_CASE("Map strings") {
        ...
        auto mapped = arr
            .iter()
            .map([](const std::string& item) { return item.size(); });
    ...
    TEST_CASE("Reduce") {
        ...
        int reduced = arr
            .iter()
            .reduce([](int& acc, const int item) { acc += item; });
    ...
    TEST_CASE("Transformations on an std::vector") {
        ...
        auto mapped = iter.filter([](int n) { return n % 2 == 1; } )
    ...
    TEST_CASE("Filter filter") {
        ...
        auto vec = to_vec(filtered);

        REQUIRE_EQ(vec, std::vector { 3, 5 });
    }
    ...
    TEST_CASE("Map map") {
        ...
        REQUIRE_EQ(vec, std::vector { 3, 5, 7 });
    }
